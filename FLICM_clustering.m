...
tic
% FLICM
% U = reshape(U,H*W,cNum);
% [U,iter] = flicm_indiana( image, U, m, cNum, maxIter, thrE );
% [U,iter] = FLICM( image, H, W, U, m, cNum, winSize, maxIter, thrE );              % c version for gary pic

[U,iter] = flicm( image, U, m, cNum, maxIter, thrE );                             % matlab version for multichannel pic

% D = 3;
% [U,iter] = FLICM_multiD( image, H, W, D, U, m, cNum, winSize, maxIter, thrE );    % c version for multichannel pic
...
Ir = zeros(size(image,1),size(image,2));    Ig = zeros(size(image,1),size(image,2));    Ib = zeros(size(image,1),size(image,2));
for i=1:cNum
    q = find( clus==i );
