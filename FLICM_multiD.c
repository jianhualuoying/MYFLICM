...
void CalcCenters( double ***image, long height, long width, long dimension, double ***U, long cNum, double m, double **c ){
	long i, j, k, n;
	double sSum;
	...
	for( k=0; k<cNum; k++ ){
		...
		for(n=0; n<dimension; n++)		//initialize 'c' every time calculate 'c'
			c[k][n] = 0.0;

		for( i=0; i<height; i++ )
			for( j=0; j<width; j++ ){
				for (n=0; n<dimension; n++){
					c[k][n] += pow(U[i][j][k],m) * image[i][j][n];
				}
			}
		for(n=0; n<dimension; n++)
			c[k][n] /= sSum;	
...
void FLICM_alg( double *img, long height, long width, long dimension, double *Uinit, double m, long cNum, long winSize, long maxIter, double thrE, double *Uout, double *iter ){
	long i, j, k, n, ii, jj, x, y, sweeps, sStep;
	double ***image, ***U, ***Uold, **c, *d, dd, *vec;
	double dMax, sSum, dist, tempSum;

	// memory allocations and initializations.
	sStep = (winSize-1)/2;
	image = mallocdbl3( height, width, dimension);
	Uold  = mallocdbl3( height, width, cNum );
	U     = mallocdbl3( height, width, cNum );
	c     = mallocdbl2( cNum, dimension );
	d     = (double*)malloc( cNum*sizeof(double) );
	for( i=0; i<height; i++ )
		for( j=0; j<width; j++ ){
			for( n=0; n<dimension; n++)
				image[i][j][n] = img[n*height*width + j*height+i];
			for( k=0; k<cNum; k++ ){
				U[i][j][k] = Uinit[k*height*width + j*height + i];
				Uold[i][j][k] = U[i][j][k];
			}
		}
	CalcCenters( image, height, width, dimension, U, cNum, m, c );
		...
		for( i=0; i<height; i++ ){
			for( j=0; j<width; j++ ){
				...
				for( k=0; k<cNum; k++ ){
					...
					sSum = 0.0;
					for(n=0; n<dimension; n++)
						sSum += pow(vec[n]-c[k][n],2);	//pow(x_i - v_k,2)
					for( ii=-sStep; ii<=sStep; ii++ )
						for( jj=-sStep; jj<=sStep; jj++ ){
							...
							if( x>=0 && x<width && y>=0 && y<height && (ii!=0 || jj!=0) ){
								...
								tempSum = 0.0;
								for(n=0; n<dimension; n++)
									tempSum += pow(vec[n]-c[k][n],2);
								sSum += 1.0/(1.0+dist)*pow(1.0-Uold[y][x][k],m) * tempSum;//pow(x_i - v_k,2)+G_ki
							}
						}
					d[k] = sSum;
				}
				...
				for( k=0; k<cNum; k++ ){
					...
					sSum = 0.0;
					for( ii=0; ii<cNum; ii++ )
						sSum += pow( dd/d[ii], 1.0/(m-1.0)); //dd/d[ii] = [pow(x_i-v_k,2)+G_ki]/[pow(x_i-v_j,2)+G_ji], j=1...cNum 
					U[i][j][k] = 1.0 / sSum;
				}
			}
		}
...
{
	int H, W, dimension, m, cNum, winSize, maxIter, dims[3];
	...
	dimension = (int)mxGetScalar(prhs[3]);
	U    = mxGetPr(prhs[4]);
	m    = (double)mxGetScalar(prhs[5]);
	cNum = (int)mxGetScalar(prhs[6]);
	winSize = (int)mxGetScalar(prhs[7]);
	maxIter = (int)mxGetScalar(prhs[8]);
	thrE    = (double)mxGetScalar(prhs[9]);
	...
	/* Do the actual work */
	FLICM_alg(img, H, W, dimension, U, m, cNum, winSize, maxIter, thrE, Uout, iter);
}
